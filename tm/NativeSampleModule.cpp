...
public:
    ...
    uint64_t addCallback(std::shared_ptr<AsyncCallback<std::string>> callback) {
        std::lock_guard<std::mutex> lock(mutex_);
        uint64_t id = nextId_++;
    ...
    void invokeCallback(uint64_t id, const std::string& data) {
        std::lock_guard<std::mutex> lock(mutex_);
...
private:
    ...
    std::mutex mutex_;
    std::map<uint64_t, std::shared_ptr<AsyncCallback<std::string>>> callbacks_;
    uint64_t nextId_;
};
...
// jsi::Value NativeSampleModule::start(jsi::Runtime& rt, std::string input) {
...
//https://github.com/facebook/react-native/issues/44813
    void NativeSampleModule::start(jsi::Runtime &rt, std::string config, jsi::Function onRes) {
    ...
    jsi::Value NativeSampleModule::startPromise(jsi::Runtime &rt, jsi::String config) {
    ...
    // works
    facebook::react::AsyncPromise<std::string> NativeSampleModule::startPromise2(jsi::Runtime &rt) {
        ...
        auto responseThunk = [](void* context, const char* data, int length) {
            __android_log_write(ANDROID_LOG_ERROR, "com.turbolnd", "kommer till responseThunk");
            auto* self = static_cast<NativeSampleModule*>(context);
    ...
    facebook::react::AsyncPromise<std::string> NativeSampleModule::startPromise3(jsi::Runtime &rt) {
    ...
    // Works but not delete
    facebook::react::AsyncPromise<std::string> NativeSampleModule::startPromise4(jsi::Runtime &rt) {
    ...
    jsi::Value NativeSampleModule::startPromise5(jsi::Runtime &rt) {
    ...
    jsi::Value NativeSampleModule::startPromise6(jsi::Runtime &rt) {
   ...
   public:
    ...
    // Does not work
    jsi::Value NativeSampleModule::startPromise7(jsi::Runtime &rt) {
        auto promiseSetup = [this](jsi::Runtime& rt, std::shared_ptr<react::Promise> promise) {
            auto context = new std::pair<jsi::Runtime*, std::shared_ptr<react::Promise>*>(&rt, std::move(&promise));

            CCallback callback = {
                .onResponse = handleOnResponseThunk,
                .onError = handleOnErrorThunk,
    ...
    //    auto promiseSetup = [this, jsCallInvoker = jsInvoker_](jsi::Runtime& rt, std::shared_ptr<react::Promise> promise) {
    //        auto safeCallback = std::make_shared<ThreadSafeCallback>(
    //            jsCallInvoker,
    //            [&rt, promise](const char* data, int length) {
    //                handleOnResponse(rt, promise, data, length);
    //            }
    //        );

    //        CCallback callback = {
    //            .onResponse = [](void* context, const char* data, int length) {
    //                auto* safeCallback = static_cast<ThreadSafeCallback*>(context);
    //                (*safeCallback)();
    //            },
    //            // ... similar for onError
    //            .responseContext = safeCallback.get(),
    //            .errorContext = safeCallback.get()
    //        };
    ...
    jsi::Value NativeSampleModule::getInfo3(jsi::Runtime &rt) {
        auto promiseSetup = [this](jsi::Runtime& rt, std::shared_ptr<react::Promise> promise) {
            // /*std::thread(*/[this, rt = std::move(&rt), promise = std::move(promise)]() {
                auto responseCallback = [this, rt = std::move(&rt), promise = std::move(promise)](const char* data, int length) {
                    __android_log_write(ANDROID_LOG_ERROR, "com.turbolnd", "kommer till responseCallback");
    ...
    jsi::Value NativeSampleModule::promise(jsi::Runtime &rt) {
        auto promiseSetup = [this](jsi::Runtime& rt, std::shared_ptr<react::Promise> promise) {
            std::thread([this, rt = std::move(&rt), promise = std::move(promise)]() {
    ...
    void NativeSampleModule::startPromiseCallback(jsi::Runtime &rt, AsyncCallback<std::string> callback) {
    ...
    void NativeSampleModule::startPromiseCallback2(jsi::Runtime &rt, AsyncCallback<std::string> callback) {
    ...
    void NativeSampleModule::startPromiseCallback3(jsi::Runtime &rt, AsyncCallback<std::string> callback) {
        auto sharedCallback = std::make_shared<AsyncCallback<std::string>>(std::move(callback));
    ...
    facebook::react::AsyncPromise<std::string> NativeSampleModule::getInfo(jsi::Runtime &rt) {
        auto* promisePtr = new facebook::react::AsyncPromise<std::string>(rt, jsInvoker_);
    ...
    facebook::react::AsyncPromise2<std::string> NativeSampleModule::getInfo4(jsi::Runtime &rt) {
    ...
    jsi::Value NativeSampleModule::getInfo2(jsi::Runtime &rt) {
    ...
    void NativeSampleModule::getInfoCallback(jsi::Runtime &rt, AsyncCallback<std::string> callback) {
        auto sharedCallback = std::make_shared<AsyncCallback<std::string>>(std::move(callback));
        uint64_t callbackId = CallbackKeeper::getInstance().addCallback(sharedCallback);
        ...
        auto unsubscribeFunc = jsi::Function::createFromHostFunction(
            ...
            [callbackId](jsi::Runtime& rt, const jsi::Value& thisValue, const jsi::Value* arguments, size_t count) -> jsi::Value {
                CallbackKeeper::getInstance().removeCallback(callbackId);
                return jsi::Value::undefined();
            }
        );
    ...
    void NativeSampleModule::reg(jsi::Runtime &rt) {
...
facebook::react::AsyncPromise<std::string> NativeSampleModule::prom(jsi::Runtime& rt) {
    auto promise = facebook::react::AsyncPromise<std::string>(rt, jsInvoker_);
    promise.resolve("result!");
    return promise;
}

void NativeSampleModule::a(jsi::Runtime &rt) {
    __android_log_write(ANDROID_LOG_ERROR, "com.turbolnd", "kommer till a");

    CCallback myCallback = {
        .onResponse = [](void *, const char *, int) {
            __android_log_write(ANDROID_LOG_ERROR, "com.turbolnd", "kommer till onResponse");
        },
        .onError = [](void *, const char *) {
            __android_log_write(ANDROID_LOG_ERROR, "com.turbolnd", "kommer till onError");
